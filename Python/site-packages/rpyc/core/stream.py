...
class Stream(object):
    ...
    def fileno(self):
        ...
        raise NotImplementedError()
    def poll(self, timeout, interrupter = None):
        """indicates whether the stream has data to read (within *timeout*
        seconds). Optionally, provide a socket object to send data to 
        preemptively interrupt the poll operation """
        timeout = Timeout(timeout)
        try:
            p = poll()   # from lib.compat, it may be a select object on non-Unix platforms
            p.register(self.fileno(), "r")
            if interrupter is not None:
                p.register(interrupter.fileno(), "r")
            while True:
                try:
                    rl = p.poll(timeout.timeleft())
                    # Poll either exited because of:
                    # * the timeout
                    # * the connection's socket 
                    # * the interrupt socket
                    # * One of the above, but both have data on them
                    # Read the data on the interrupt socket if any.
                    if interrupter and rl and (rl[0][0] == interrupter.fileno() or \
                       (len(rl) == 2 and rl[1][0] == interrupter.fileno())): 
                        interrupter.recv(1)
                except select_error:
                    ex = sys.exc_info()[1]
        ...
        # We weren't woken up by the connection's socket, return false
        if interrupter and len(rl) == 1 and rl[0][0] == interrupter.fileno():
            rl = False
        # We were woken up by the socket, cast the result to bool
        else:
            return bool(rl)
    def read(self, count):
