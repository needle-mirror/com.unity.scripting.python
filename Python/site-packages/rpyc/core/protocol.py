...
class Connection(object):
    ...
    def __init__(self, root, channel, config={}):
        ...
        # Make a socket (preferably a unix domain socket).
        import rpyc.lib.compat
        try:
            if hasattr(rpyc.lib.compat, 'unix_socket'):
                # Make a random filename for a socket and clean it up.
                interrupt_listen_name = os.path.join( tempfile.tempdir,
                    'rpyc_'+''.join(random.choice(string.ascii_uppercase + string.digits) for _ in xrange(10)))+'.socket'
                l = rpyc.lib.compat.unix_socket(socket.AF_UNIX)
                l.bind(interrupt_listen_name)
                self._interrupt_socket_send = rpyc.lib.compat.unix_socket(socket.AF_UNIX)
            else:
                l = socket.socket()
                l.bind(('localhost', 0))
                self._interrupt_socket_send = socket.socket()
        
            l.listen(1)
            self._interrupt_socket_send.connect(l.getsockname())
            self._interrupt_socket_send.setblocking(0)
            self._interrupt_socket_recv, _ = l.accept()
            self._interrupt_socket_recv.setblocking(0)
        finally:
            l.close()
            if hasattr(rpyc.lib.compat, 'unix_socket'):
                try:
                    os.remove(interrupt_listen_name)
                except IOError:
                    pass
    ...
    def _cleanup(self, _anyway = True):
        ...
        #self._config.clear()
        self._interrupt_socket_send.close()
        self._interrupt_socket_recv.close()
        del self._HANDLERS
    ...
    def serve(self, timeout=1, wait_for_lock=True):
        ...
        data = None
        try:
            data = self._channel.poll(timeout, self._interrupt_socket_recv) and self._channel.recv()
            if not data:
                return False
        except EOFError:
            self.close()
        ...
        self._dispatch(data)
        # In multithreaded scenarios with a threaded server, if another thread
        # than the server makes a request to a client, the timing may become 
        # such that the server thread processes the client's response, causing 
        # the other thread to lock on poll until the timeout times out. Write 
        # one byte on the pipe (the other side is passed to the poll method 
        # above) to preempt the end of the poll. 
        # See:
        # https://github.com/tomerfiliba/rpyc/issues/354
        # https://github.com/tomerfiliba/rpyc/issues/345
        self._interrupt_socket_send.send(b'-')
        return True
